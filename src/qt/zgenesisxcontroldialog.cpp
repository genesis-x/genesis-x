...
#include "ui_zgenesisxcontroldialog.h"
...
std::list<std::string> ZGenesisXControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGenesisXControlDialog::listMints;

ZGenesisXControlDialog::ZGenesisXControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGenesisXControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGenesisXControlDialog::~ZGenesisXControlDialog()
{
    delete ui;
}

void ZGenesisXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGenesisXControlDialog::updateList()
{
...
void ZGenesisXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGenesisXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGenesisX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGenesisXControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGenesisXControlDialog::GetSelectedMints()
{
...
void ZGenesisXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
